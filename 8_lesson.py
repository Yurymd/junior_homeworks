currencies = [{"Cur_ID":440,"Date":"2021-10-11T00:00:00","Cur_Abbreviation":"AUD","Cur_Scale":1,"Cur_Name":"Австралийский доллар","Cur_OfficialRate":1.8082},
			  ...
			  {"Cur_ID":371,"Date":"2021-10-11T00:00:00","Cur_Scale":1,"Cur_Name":"Канадский доллар","Cur_OfficialRate":1.9754},
...
def get_cur_by_abbr(currencies, abbr):
	for kurs in currencies:
		if kurs.get ("Cur_Abbreviation") == abbr:
			return kurs
	print(f"Для валюты {abbr} результат не найден")

def create_message_about_curr(cur):
	if type(cur) == dict:
		abbreviation = cur.get ("Cur_Abbreviation")
		rate = cur.get ("Cur_OfficialRate")
		scale = cur.get ("Cur_Scale")
		message = f'Курс {abbreviation} на сегодня: {rate} рубля за {scale} {abbreviation}.'
		return message

def send_message(currencies, curr):
	currency = get_cur_by_abbr(currencies, curr)
	if currency:
		message = create_message_about_curr(currency)
		print(message)

send_message(currencies, curr)
